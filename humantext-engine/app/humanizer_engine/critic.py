...
def critique(text: str, before: Profile) -> list[str]:
    ...
    if ":" in text and any(w in text.lower() for w in ("topic:", "lesson:", "note:", "important topic:", "everywhere:")):
    ...
    if re.search(r"\b(?:it\'s|it is)\s+(?:really\s+)?all about\b", text, re.IGNORECASE):
        issues.append("Eliminate repetitive 'It's all about...'. Describe the core idea directly using varied sentence subjects.")
    if re.search(r"\b(?:in today\'s (?:world|tech-driven world|digital age))\b", text, re.IGNORECASE):
        issues.append("Remove 'In today's world' cliché. Use natural conversational phrasing.")
    if re.search(r"\b(?:like\s+[a-z\s]+,\s*[a-z\s]+,\s*[a-z\s]+,\s*[a-z\s]+,\s*and\s+[a-z\s]+)\b", text, re.IGNORECASE):
        issues.append("Eliminate 5-item comma list. Summarize the options naturally into 1-2 examples.")
